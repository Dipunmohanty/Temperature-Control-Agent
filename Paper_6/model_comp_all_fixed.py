...
def generate_data(n=3000, complexity=0):
    t = np.arange(n)
    ...
    # Random spikes (scaled with data size)
    if complexity >= 2:
        spikes = np.zeros(n)
        num_spikes = int(0.02 * n)  # 2% spikes
        idx = np.random.choice(n, size=num_spikes, replace=False)
        spikes[idx] = np.random.uniform(-6, 6, num_spikes)
        outside += spikes
...
def train_lstm(X_train, y_train, X_test):
    ...
    start = time.time()
    model.fit(Xn, yn, epochs=20, verbose=0)
    duration = time.time() - start
...
for c in complexities:
    ...
    # Models
    lw_preds, lw_time = train_lightweight(X_train, y_train, X_test)
    ridge_preds, ridge_time = train_model(Ridge(alpha=1.0), X_train, y_train, X_test)
    tree_preds, tree_time = train_model(DecisionTreeRegressor(max_depth=5), X_train, y_train, X_test)
    mlp_preds, mlp_time = train_model(MLPRegressor(hidden_layer_sizes=(32,), max_iter=300), X_train, y_train, X_test)
    ...
    plt.plot(y_test[-plot_size:], label="Actual")
    plt.plot(lw_preds[-plot_size:], label="Lightweight")
    plt.plot(ridge_preds[-plot_size:], label="Ridge")
    plt.plot(tree_preds[-plot_size:], label="Tree")
    plt.plot(mlp_preds[-plot_size:], label="MLP")
    plt.plot(lstm_preds[-plot_size:], label="LSTM")
